...
class IndexScheduler
{
    ...
    {
        $corpus = ($corpus instanceof CorpusInterface) ? $corpus->getName() : $corpus;
        return $this->getScheduledIndexRepository()->findBy([
            'status' => ScheduledIndex::SCHEDULED,
            'corpus' => $corpus,
        ]) ?: [];
    }
    ...
    public function getProcessingExports($corpus)
    {
        $corpus = ($corpus instanceof CorpusInterface) ? $corpus->getName() : $corpus;
        return $this->getScheduledIndexRepository()->findBy([
            'status' => ScheduledIndex::PROCESSING,
            'corpus' => $corpus,
        ]) ?: false;
    }
    ...
    public function failExpiredProcessingExports($corpus)
    {
        ...
        $this->logger->error(sprintf(
            'Scheduled index contained a processing entry older than %s minutes. This was set to failed. Please investigate why the indexing process is taking too long or failing without moving to `failed` status.',
            self::PROCESSING_EXPIRY
        ));
