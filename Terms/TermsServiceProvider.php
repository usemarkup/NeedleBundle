...
/**
 * A provider of terms services, using the Symfony DI container.
 */
class TermsServiceProvider
{
    /**
    ...
    public function getServiceForAlias($alias)
    {
        if (!isset($this->services[$alias])) {
            throw new \InvalidArgumentException(
                sprintf('There is no registered terms service with the alias "%s".', $alias)
            );
        }
        if (!$this->container->has($this->services[$alias])) {
            throw new \InvalidArgumentException(
                sprintf(
                    'The service "%s" referred to by the alias "%s" is not registered with the Symfony DI container.',
                    $this->services[$alias],
                    $alias
                )
            );
